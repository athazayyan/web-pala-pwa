...
const responses: Record<string, { text: string; product: Product }> = {
  "I have trouble sleeping": {
    text: "To ease the mind and prepare the body for rest, I recommend the **Premium Nutmeg Night Balm**. Its myristicin compounds act as a natural sedative when applied to the temples.",
    product: {
      name: "Night Balm",
  ...
  "I need an energy boost": {
    text: "For clarity and focus, our **Warm Nutmeg Syrup** infused with Tapaktuan ginger is ideal. It stimulates circulation without the caffeine crash.",
    product: {
      name: "Warm Syrup",
  ...
  "Sooth muscle aches": {
    text: "Our **Apothecary Grade Nutmeg Oil** is a potent anti-inflammatory. Dilute three drops in a warm bath or carrier oil to release tension.",
    product: {
...
export function Services() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      text: `"Selamat datang. I am the Apoteker Rempah. From the ancestral wisdom of Aceh Selatan, I bring you the healing properties of our finest Nutmeg. How may I assist your well-being today?"`,
      isUser: false
    }
  ])
  ...
  const [activeCategory, setActiveCategory] = useState("Sleep & Relaxation")
  ...
  const handleSend = (text: string) => {
    ...
    setTimeout(() => {
      ...
      const botMsg: Message = matchedResponse 
        ...
        : {
            id: `bot-${Date.now()}`,
            text: "That is an interesting concern. Our Tapaktuan harvest is rich in many compounds. May I suggest looking into our Sleep or Vitality categories for a natural remedy?",
            isUser: false
          }
      ...
      <aside className="hidden lg:flex flex-col w-80 bg-surface-container-low border-r border-outline-variant p-8 gap-8 overflow-y-auto">
        <div>
          ...
          <ul className="space-y-2">
            {[
              { name: "Sleep & Relaxation", icon: "nights_stay" },
              { name: "Natural Vitality", icon: "bolt" },
              { name: "Apothecary Blends", icon: "spa" },
              { name: "Focus & Clarity", icon: "self_care" }
            ].map((cat) => (
              <li key={cat.name}>
                <button 
          ...
          <div className="space-y-4">
            <div className="group cursor-pointer">
              <div className="h-32 w-full rounded-xl mb-2 overflow-hidden">
                <div 
                  ...
                  style={{ backgroundImage: `url('https://lh3.googleusercontent.com/aida-public/AB6AXuDefJb0D_YxwnTy9vrcBW6TNkUDjJTlvkWFDrgtoW-uq_dk5ZZdwhEZl_w8YXUTkwdZJvAt2ADr2Ag0E4p3E-QfhOJtHEI8FywXrfNiY_5_GUMQnomtsCWaZsR7815WVIaGFn8xfdxGIKiJACZWE7AVY7KbZpxxT9vH2ue97AyIEZ8UcjEG7yUTBvVxLSelnbcL17CoVpUh6dn7KZcK13YbGLT-oQLbByC3gxBnjnrTqI089hCIjQ')` }}
                />
              </div>
              <p className="font-display-b2c text-body-md text-primary">Nutmeg Sleep Salve</p>
              <p className="font-body-sm text-label-caps text-outline">Tapaktuan Reserve</p>
            </div>
            ...
            <div className="group cursor-pointer">
              <div className="h-32 w-full rounded-xl mb-2 overflow-hidden">
                <div 
                  className="bg-cover bg-center w-full h-full group-hover:scale-105 transition-transform duration-500" 
                  style={{ backgroundImage: `url('https://lh3.googleusercontent.com/aida-public/AB6AXuDL7LgR4WyThUv9TIn2eH0jl1DtVHsEngS7URgkx1443jiNCXW21QPhtco1Z4bNpr7weU24zq_Z3zvIf9kOrw_UUJgobP5xJe1Jd8jtuZBevIrV1wZWrk8BAUeV2moMXAf4TL0jcK9lWdPgf-qrOCnx1uW9-qnDg7wIPmZTE4G5IFYFuUYB_V7-9_pQFV68N6oCdmEytVAKGKu2FabW5c12DOGCtWO6d_mykRNfN7yB1Bngce7LKw')` }}
                />
              </div>
              <p className="font-display-b2c text-body-md text-primary">Vitality Elixir</p>
              <p className="font-body-sm text-label-caps text-outline">Pure Extraction</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <section className="flex-grow flex flex-col relative bg-surface-bright">
        ...
        <div className="p-6 border-b border-outline-variant/30 flex justify-between items-center bg-surface/50 backdrop-blur-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-body-sm text-label-caps text-on-surface-variant/70">Your Personal Nutmeg Consultant</p>
            </div>
          </div>
        ...
        <div className="flex-grow overflow-y-auto chat-scroll p-8 space-y-8">
          {messages.map((msg) => (
            <div key={msg.id} className="space-y-4">
              <div className={`flex gap-4 ${msg.isUser ? "justify-end animate-in fade-in slide-in-from-bottom-4 duration-500" : "justify-start"}`}>
                {!msg.isUser && (
              ...
              {msg.product && (
                <div className="flex gap-4 ml-12 animate-in fade-in zoom-in duration-700 delay-300">
                  <div className="bg-white border border-outline-variant/30 rounded-2xl p-4 shadow-sm flex items-center gap-4 max-w-sm">
                    ...
                    <div className="flex-grow">
                      ...
                      <button className="mt-2 text-label-caps font-label-caps text-secondary hover:text-on-secondary-container flex items-center gap-1">
                        ADD TO CART <span className="material-symbols-outlined text-xs">arrow_forward</span>
                      </button>
                    </div>
                  </div>
                </div>
          ...
          {isTyping && (
            <div className="flex gap-4 justify-start">
              ...
              <div className="bg-surface-container-low p-4 rounded-tr-2xl rounded-br-2xl rounded-bl-2xl border border-outline-variant/20 shadow-sm text-xs font-mono text-on-surface-variant">
                Apoteker Rempah is formulating a remedy...
              </div>
            </div>
          )}

          {/* Quick options at the beginning */}
          {messages.length === 1 && (
            <div className="flex flex-wrap gap-2 ml-12">
              <button 
                onClick={() => handleSend("I have trouble sleeping")}
              ...
              <button 
                onClick={() => handleSend("I need an energy boost")}
                className="px-4 py-2 rounded-full border border-outline-variant bg-surface-container-lowest text-on-surface-variant font-label-caps text-label-caps hover:bg-tertiary-fixed hover:border-tertiary transition-all"
              >
                Need an energy boost
              </button>
              <button 
                onClick={() => handleSend("Sooth muscle aches")}
                className="px-4 py-2 rounded-full border border-outline-variant bg-surface-container-lowest text-on-surface-variant font-label-caps text-label-caps hover:bg-tertiary-fixed hover:border-tertiary transition-all"
              >
                Sooth muscle aches
              </button>
        ...
        <div className="p-8 pt-0">
          ...
          <p className="text-center mt-4 font-label-caps text-label-caps text-on-surface-variant/50">
            Wisdom-based recommendations. Consult a physician for chronic conditions.
          </p>
        </div>
      </section>
    </div>
